...
(https://drive.google.com/file/d/136xLr8zlhSKE6idA1UEtD914SDXjoDhM/view?usp=sharing)
...
R12941077:「這次專題最大的挑戰在於如何讓記帳變得『無腦』且『精準』。我們導入了 Google Gemini 1.5 Flash 來處理自然語言解析，但在開發初期發現，LLM 雖然能讀懂文字，但要穩定輸出符合資料庫格式的 JSON 卻常出錯。我們花了大量時間微調 Prompt，並結合 Zod 進行嚴格的 Schema 驗證，才確保每一筆『午餐 150』都能正確存入 MongoDB。

另外，這也是我們第一次嘗試 Next.js 16 與 LINE Messaging API 的深度整合。要讓 LINE Bot 記錄的資料與 Web Dashboard 即時同步，並處理 Webhook 的安全性驗證，確實讓我們踩了不少坑。雖然過程很累，但看到數據能跨平台流暢運作，真的很有成就感！」


----

R13944039:「記帳最難的不是工具，而是『堅持』。因此我們在設計這款機器人時，決定加入電子雞養成系統。我們希望透過遊戲化的回饋，讓使用者覺得記帳不再是瑣事，而是餵食寵物的過程。

在設計 Web Dashboard 時，我們選擇了 MUI 7 與 Recharts，目標是提供比 LINE 介面更深度的財務分析，像是消費趨勢圖與預算超支警告。為了兼顧美觀與直覺，我們反覆調整了響應式佈局。雖然功能開發壓力很大，但最後看到電子雞進化與漂亮的統計圖表成形時，我們深切體會到產品設計中『趣味性』與『實用性』平衡的重要性。」

----

B11705055:「對於平時較常接觸 Python 或研究資料的我來說，這次跨足到 TypeScript 與 Next.js 全端開發是一個巨大的轉變。從處理 LINE 的 Webhook 串接，到使用 NextAuth.js 實作 Google 登入驗證，每一塊拼圖都學到了很多新的概念。

特別是在處理群組分帳功能時，邏輯的複雜度超乎預期，必須考慮到不同成員的出資與應收金額。雖然開發期間正好撞到實驗室的研究計畫最忙的時候，但與組員共同討論解決方案、一起除錯到深夜的過程，讓我們對網頁程式設計有了更實戰的體會，這是在課堂講義上學不到的珍貴經驗。」
