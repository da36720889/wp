...
- 💰 **預算管理與提醒**：設定單日/單週/單月預算上限，超支時自動提醒
- 👥 **群組分帳功能**：在 LINE 群組中記錄共同支出，自動計算分攤金額和轉帳建議
- 📧 **Gmail 記帳匯入**：自動解析 Gmail 中的消費通知郵件，匯入記帳記錄
- 🎯 **儲蓄目標管理**：設定儲蓄目標，追蹤達成進度
...
3. **查詢記錄**：
   - 輸入 `/list` 或 `/list 20` 查看最近的記帳記錄

4. **查看摘要**：
   - 輸入 `/summary` 查看總收入、總支出、餘額
